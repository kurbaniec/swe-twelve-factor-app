...
ADD https://storage.googleapis.com/tensorflow/libtensorflow/libtensorflow-cpu-linux-x86_64-2.9.1.tar.gz /usr/src/dogorcat-app/ /usr/src/dogorcat-app/tf/
WORKDIR /usr/src/dogorcat-app/
...
CMD ["/bin/bash", "-c", "export LD_LIBRARY_PATH=/usr/src/dogorcat-app/tf/libtensorflow/lib;export ROCKET_LIMITS='{form=100000000,forms=100000000,data-form=100000000,file=100000000}';export DATABASE_URL='postgres://postgres:postgres@localhost/app-db';diesel run"]